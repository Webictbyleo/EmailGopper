...
class Convert {
	...
	public static function raw2sql($val) {
		if(is_array($val)) {
			...
			return MysqliDb::getConn()->escape($val);
		}
	}
	...
	public static function tostring($str){
		if(is_Object($str))
		return ArrayLib::toString(get_object_vars($str));
		if(is_array($str)){
		return ArrayLib::toString(ArrayLib::flatten($str));
		}
		return (string)''.$str.'';
	}
	public function toInt($str){
	$yep = explode('|','affirmative|all right|aye|indubitably|most assuredly|ok|of course|okay|sure thing|y|yes+|yea|yep|sure|yeah|true|t|on|oui|vrai|1|true|yes');
        $no = explode('|','no*|no way|nope|nah|na|never|absolutely not|by no means|negative|never ever|false|f|off|non|faux|0|false');
		
	$val = $str;
	if(in_array($val,$yep,true)){
	$val = 1;
	...
	public function tobool($str){
		$yep = explode('|','affirmative|all right|aye|indubitably|most assuredly|ok|of course|okay|sure thing|y|yes+|yea|yep|sure|yeah|true|t|on|oui|vrai|1|true|yes');
        $no = explode('|','no*|no way|nope|nah|na|never|absolutely not|by no means|negative|never ever|false|f|off|non|faux|0|false|no');
		
		if(in_array($str,$yep,true)){
		return true;
		}elseif(in_array($str,$no,true)){
		return false;
		}
		return $str;
	}
	public function toarray($str){
		return ArrayLib::flatten(array($str));
	}
}
