...
class HTTPResponse {
	...
	if(Registry::getInstance()->isRegistered('headers')){
	$header = ArrayLib::array_merge_recursive($header,Registry::getInstance()->get('headers'));
	}
	
	foreach($header as $h=>$value){
	...
	//Method for generating temporary response
	public function mimic(){
	 $try = WebBaker::get_package_controller_line();
	 $try = ($try);
	...
	public function output() {
		...
		if (is_string($this->getStatusDescription())) print_r(wordwrap($this->getStatusDescription(), 100));
		else print_r($this->getStatusDescription());
		echo '</pre>';
			} else {
			Registry::getInstance()->set('before_render_complete',true);
