...
class ArrayLib {
	...
	public static function toString(array $array,$attr=false){
		if (!is_array($array)) {
			return false;
		}
	...
	if(count($key)>count($vals))return;
	$delimi = '&';
		if($attr)
		$delimi = " ";
	for($int =0;count($key)>$int;$int++){
	
	if($int == count($vals)-1 )$delimi = '';
		if($attr ==true){
		$out[] = $key[$int].'="'.$vals[$int].'"'.$delimi;
		}else{
			if(is_array($vals[$int])){
			$vals[$int] = implode(',',$vals[$int]);
			}
	$out[] = urlencode($key[$int]).'='.urlencode($vals[$int]).$delimi;
	}
	}
		if(is_array($out)){
	return implode('',$out);
