...
class HTTPRequest  {
	...
	const BROWSER_TRACK_KEY = 'hhhe64674yriur7y38y';
	 ...
	 * @return array
	 */
	public function requestVars($key=null) {
			if($key !==NULL){
				return $this->requestVar($key);
			}
	...
	public function getURL($includeGetVars = false) {
		...
		if ($includeGetVars) { 
			...
			if (count($vars) and strpos($url,'?') ===false) {
	...
	public function getTrack($track='browsers'){
	Tracker::setOwner($track);
Tracker::setprimaryKey(md5(self::BROWSER_TRACK_KEY));
$hit = Array_count_values(ArrayLib::array_column(Tracker::parseTrack(md5(self::BROWSER_TRACK_KEY)),'entity'));
return $hit;
	}
	...
	public static function send_file($fileData, $fileName, $mimeType = null) {
		...
		$response->addHeader(array("Content-Type"=>"$mimeType; name=\"" . addslashes($fileName) . "\"",
	...
	public function has_extenstion(){
...
public function init_has_extension($ext){


}
//Exchnage cintroller data

public function init_exchange($ext){

}
public function init_extension($ext){
